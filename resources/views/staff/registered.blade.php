  ...
  <div class="mx-10 justify-center" >
    <form action="{{ route('staff.registered') }}" method="get" class="my-10">
        ...
        <div class="relative">
            ...
            <input autocomplete="off" type="search" name="keyword" value="{{ $keyword }}" id="keyword" class="block w-full p-3 px-10 mt-3 text-gray-700 shadow border border-[#e5e5e5] rounded-lg bg-[#f2f0f0] focus:ring-blue-500 focus:border-blue-500 md:py-4" placeholder="" required>
    ...
    @foreach($employees as $employee)
      <div class="block w-full p-3 py-4 my-4 text-sm text-gray-700 shadow border border-[#e5e5e5] rounded-lg bg-white md:text-base md:p-3 md:py-6" >
          <div class="grid grid-cols-5 gap-4 content-start">
              <p class="my-2 ml-4 mobile:text-xs">{{ $employees->firstItem() + $loop->index }}</p>
              <p class="my-2 mobile:text-xs break-words">{{ $employee->name }}</p>
              <p class="my-2 mobile:text-xs break-words">{{ $employee->organizer->name }}</p>
              <p class="my-2 mobile:text-xs break-words">{{ $employee->email }}</p>
              <p class="my-2 mobile:text-xs break-words">{{ $employee->timeFormat($employee->register_at) }}</p>
          </div>
      </div>
    @endforeach
