  ...
  <div class="mx-10 justify-center">
    ...
    <div class="block w-full p-3 py-4 my-4 text-sm text-gray-700 shadow border border-[#e5e5e5] rounded-lg bg-white md:text-base md:p-3 md:py-6" >
        <div class="grid grid-cols-3 gap-4">
            <p class="text-xs sm:text-sm md:text-base my-2 ml-3">{{ $employee->name }}</p>
            <p class="text-xs sm:text-sm md:text-base my-2">{{ $employee->organizer->name }}</p>
            <div class="flex mt-1.5 mb-0.5 items-top sm:items-center sm:my-0 justify-end">
            <button class="text-white text-xs md:mr-2 sm:text-sm md:text-base w-max h-min p-2 sm:px-3 md:px-4 shadow rounded-lg bg-[#B0C03B] hover:bg-[#98a534]">
                ลงทะเบียน
            </button>
            </div>
        </div>
